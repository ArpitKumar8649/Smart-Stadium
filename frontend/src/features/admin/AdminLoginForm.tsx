import { logger } from "../../lib/telemetry.ts";
import { useState } from 'react';
import { Wordmark } from '../../components/brand/Logo.tsx';
import { signInWithPopup } from 'firebase/auth';
import { auth, googleProvider } from '../../lib/firebase.ts';

interface AdminLoginFormProps {
  error: string | null;
  clearError: () => void;
}

export function AdminLoginForm({ error, clearError }: AdminLoginFormProps) {
  const [loading, setLoading] = useState(false);
  const [localError, setLocalError] = useState<string | null>(null);

  const handleGoogleSignIn = async () => {
    setLoading(true);
    setLocalError(null);
    clearError();
    try {
      await signInWithPopup(auth, googleProvider);
    } catch (err: unknown) {
      logger.error("Login failed:", err);
      setLocalError(err instanceof Error ? err.message : 'Failed to sign in with Google');
    } finally {
      setLoading(false);
    }
  };

  const displayError = error || localError;

  return (
    <main id="main-content" tabIndex={-1} className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-4 rounded-2xl border border-surface-800 bg-surface-900 p-6 shadow-xl text-center">
        <Wordmark className="justify-center" />
        <p className="text-sm text-surface-400">Tournament Operations Console</p>
        <p className="text-xs text-surface-500">Venue staff &amp; organizers · FIFA World Cup 2026</p>

        <div className="pt-4">
          <button
            type="button"
            disabled={loading}
            onClick={handleGoogleSignIn}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-surface-50 px-4 py-3 font-semibold text-surface-950 transition hover:bg-surface-200 disabled:opacity-60"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
              <path d="M1 1h22v22H1z" fill="none" />
            </svg>
            {loading ? 'Signing in…' : 'Sign in with Google'}
          </button>
          {displayError && <p className="mt-4 text-xs text-red-400" role="alert">{displayError}</p>}
        </div>
      </div>
    </main>
  );
}
